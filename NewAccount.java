...
public class NewAccount extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        UserBean bean = new UserBean(email);
        boolean created = bean.insertAccount(fname, lname, pass, ship, bill, card);
        if(created){
            HttpSession session = request.getSession();
            session.setAttribute("UserBean", bean.getUsername());
            session.setAttribute("PrivBean", bean.getPriv());
            //session.setAttribute("ShoppingCart", shoppingCart);
            request.getRequestDispatcher("WelcomePage.jsp").include(request, response);
        }else{
